...
const CONFLICT_AI = `<div class="md" style="max-width:none;font-size:13.5px"><p style="margin:0">A two-chair salon in Kolkata that loses about four bookings a week to double-booking and no-shows. <span class="chg">The owner runs everything from a phone, has never used a laptop for the business, and the front desk closes at eight.</span></p></div>`;
...
<div class="split"><div class="pane"><div class="rh">${ic('devices', 14)} This browser · you · today 14:02</div>${CONFLICT_L}<div style="margin-top:14px"><span class="btn primary">${ic('check', 15)} Accept this version</span></div></div><div class="gut"></div>
<div class="pane"><div class="rh">${ic('add_to_drive', 14)} Google Drive · Amit · today 14:05</div>${CONFLICT_R}<div style="margin-top:14px"><span class="btn primary">${ic('check', 15)} Accept this version</span></div></div></div>
<div style="margin:0 28px 10px;border:1px solid var(--ai);border-radius:10px;padding:10px 14px;background:var(--panel-2)"><div class="rh">${ic('auto_awesome', 14)} AI suggestion · a preview, nothing is written yet · one change in your queue</div>${CONFLICT_AI}</div>
<div style="padding:10px 28px 16px;display:flex;gap:10px;align-items:center;flex-wrap:wrap;border-top:1px solid var(--border)"><span class="btn">${ic('content_copy', 15)} Keep both as two files</span><span class="btn">${ic('auto_awesome', 15)} Let AI decide</span><span class="btn ai">${ic('check', 15)} Accept AI suggestion</span><span class="btn ghost">${ic('checklist', 15)} Review span by span</span><span style="flex-basis:100%;font-size:12px;color:var(--fg-muted)">Whichever you choose, the other version stays in history. The same screen appears for a desktop edit against a GitHub change.</span></div>
</main></div></div>`,
...
<div class="pdoc" style="padding:12px 14px 0"><div class="rh">${ic('devices', 14)} This phone · you · 14:02</div>${CONFLICT_L}<span class="btn primary" style="width:100%;margin:8px 0 16px">${ic('check', 15)} Accept this version</span>
<div class="rh">${ic('add_to_drive', 14)} Google Drive · Amit · 14:05</div>${CONFLICT_R}<span class="btn primary" style="width:100%;margin:8px 0 10px">${ic('check', 15)} Accept this version</span>
<div style="display:grid;grid-template-columns:1fr 1fr;gap:6px"><span class="btn">${ic('content_copy', 15)} Keep both</span><span class="btn ai">${ic('auto_awesome', 15)} Let AI decide</span></div><span class="btn" style="width:100%;margin-top:6px;opacity:.5">${ic('check', 15)} Accept AI suggestion</span><div style="font-size:12px;color:var(--fg-muted);margin-top:6px">Accept AI suggestion opens once Let AI decide has shown you its merge. The other version stays in history.</div></div>` }));
