...
Checked on 18 September: **it does not.** `[O]` `12-screens/S20.md` gives a queue row the shape
`{ id, source, author, ask, span, proposedBytes, at }`, one span per row, and its Accept is one splice
per row. So S20 has to gain a list of spans inside one item, with accept and reject per span, before
`A681` can pass. That change belongs to S20's owner.
...
**`listConflicts` exists in the code and is not this screen's source.** `[O]` Read on 18 September
at `e0f6f89`: `src/modules/share/application/list-conflicts.ts` returns `SlugConflict`, a public slug
claimed by more than one note. It knows nothing of versions. `ConflictReader` is new work, and its
record is `67-SYNC-AND-CONFLICT.md` section 5.7.
...
unauthorised | The person has no Apply right, per `D67` | The conflict is shown read-only, with who may resolve it | Read, and comment if their role allows
...
three-way | More than two versions diverged | The head on the left, one side at a time on the right, with a side 1 of 2 switch. Each side resolves on its own against the current head. Specified, not drawn. See `D65` | Resolve each side in turn
...
- `[O]` No conflict with `15-INTERACTION-AND-KEYBOARD.md`, read on 18 September. Its section 6.1
  row for S31 puts focus on the heading and never on a version, which is this screen's banner rule,
  and section 2 assigns no chord here.
...
`D65` | What happens when three or more versions diverge? | Two panes are drawn and the plan describes two. A third device makes this screen wrong rather than crowded | **resolved (proposed 18 Sep, founder review).** Adopted from `67-SYNC-AND-CONFLICT.md` section 6: extra sides append to one conflict record, S31 keeps two panes and shows one side at a time against the head, and `conflictOpen` clears when no side is left. Rejected: three panes, which do not fit the phone, and pre-folding sides, which is a merge
`D66` | **Decided 18 September 2026.** What did the founders mean by **Accept**? | `[Z]` Accept is the per-side control, renamed from Keep this one to Accept this version. Accept AI suggestion is a control on this screen, beside Let AI decide, that accepts the one previewed proposal. Recorded in Anatomy, Actions, States and `A726` to `A728` above | Already decided `[Z]`. No change
`D67` | Who resolves a conflict on a shared document, the owner or any editor? | `docs/mvp0/PRODUCT-PLAN.md` section 19 gives Editor both Propose and Apply, which implies an editor may resolve. The plan does not say it | **resolved (proposed 18 Sep, founder review).** Anyone holding Apply in that table: Owner, Editor and a link-edit holder. Commenter and Viewer see it read-only, and an agent token never resolves, since Apply is never. Rejected: owner only, which strands a shared document while the owner is away
`D68` | What are the two files called after Keep both? | The control is specified, the naming is not, and a silent rename would be its own small violation | **resolved (proposed 18 Sep, founder review).** The head keeps its name. The side copy is `<stem> (from <origin>).md`, with the origin word the pane header shows. Both names are shown on the control before it is pressed. If the name is taken, `E550` asks. Rejected: a number suffix, which does not say which is which
