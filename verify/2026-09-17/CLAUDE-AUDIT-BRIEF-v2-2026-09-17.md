# Audit brief v2 for Claude: the frontmatter product plan v4, every context, every angle, at full depth
...
## 0. Context you must hold before you start

Everything in this section was recorded by the author's sessions and is itself under audit. Verify what you rely on. Pointers are given so you can.

**The studio.** Studio Zephyrus, two founders, Sagnik Mitra and Amit Kumar, Kolkata. A freelance agency that also builds products. The founders' measured pace on this product has been about 1.2 days a week between them; the plan's own words are that the scope exceeds two part-time founders. The studio's other products and assets that the plan reuses: md.sgnk.ai (the editor this product grows from, the design reference), advox (33 documentation files of 119,284 words with a knowledge graph of 4,600 nodes and 6,367 edges, built with graphify), lumiera (a complete Razorpay integration), CareerOS (a credit engine with plan entitlements and kill switches), the decisions site's ten diagram primitives, pdf.sgnk.ai, skills.sgnk.ai (184 skills). No repository in the studio has an offline outbox. Source: `verify/2026-09-17/research/2026-09-16-agent9-knowledge-assets.md`.

**Accounts and ownership.** GitHub `studiozephyrus/frontmatter`. Vercel team `zsco` for the app and a separate team for the decisions site. Firebase project `frontmatter-md` under a studio Gmail account. The Cloudflare zone for `frontmatter.in` on a founder's personal account, by deliberate choice. The domain was taken on 24 June 2026. Source: `AGENTS.md` section 6b. Do not open the token file these sections mention.

**The timeline.**

- 29 July 2026: the format verdict. Build a compiler and an editor, not a new markdown format. `HANDOFF-mdz-markdown-format-2026-07-29.md` if present, and `CLAUDE.md` "Settled".
- 29 August 2026: the technical PRD v2, `docs/FRONTMATTER-PRD-v2-2026-08-29.md`, 61 sections. Its section 57 is a corrections ledger and its section 32 a contradictions ledger of numbers that went stale. Grep them; do not open the file whole.
- 6 to 8 September: the product was framed as "review state" (see what the agent changed, know what nobody reviewed), then failed an adversarial round on 8 September. `docs/GAPS-2026-09-08.md`.
- 9 September: 204 decision cards finished; nine research lenses found that Almanac had shipped this exact feature set with read receipts, raised $45 million and shut down on 31 January 2025; that Zed's Delta stakes span attribution on CRDTs against the studio's settled position; that the attention literature is hostile to an unreviewed-percentage badge; that a research agent had fabricated quotations from VS Code documentation; that review state did not exist in the code and the engine was unwired; that `GITHUB_REPO` defaulted to a sibling project's vault. `docs/research/2026-09-09/BRIEFING.md` and `VERIFIED-2026-09-09.md`.
- 13 September: the founders reset the product to a bare editor with md.sgnk.ai's look plus a document-kit generator as the funnel, gated on twenty hand-made kits. `docs/MVP-PLAN-2026-09-13.md`.
- 15 and 16 September: the founders' answers produced MVP 0 plan v1, v2, v3 and a print build sheet, revision 3. Twelve research agents ran. `docs/mvp0/MVP0-PLAN-v3.md`, `docs/mvp0/MVP0-PLAN-print.md`, the reports dated 2026-09-16.
- 17 September: the founders changed the front door to sign-in first, asked for the features listed in A2, and nine more research branches ran. The plan v4 under audit, commit `7b49f84`; the screens corrected at `e0ac6fa`.

**Settled by the founders, recorded in `CLAUDE.md` as not to be re-litigated.** The format verdict. The render carrier: a callout for prose, a fenced block for opaque data. Sync is git-merge plus a splice journal and compare-and-swap, never a CRDT, with a written rebuttal to Zed Delta still owed. R2 has no object versioning, so disaster recovery lives in the key layout. ₹15,000 per transaction is an RBI constant and Indian cards get one payment attempt. You may challenge a settled point, but label the challenge as such and bring new evidence, not preference.

**The founders' own words, verbatim, for the bias and traceability angles.** "I don't want these captchas and all. I want a very simple, simple-to-use tool. With great features, great feedback on UX and all, a great editor that feels like home." "It will be like google docs, you have to sign in first to get into the site and editor." "Without sign-in, you won't be able to do anything." "We have to make the best freaking note-taking app, the best freaking markdown note-taking app in the world. Whatever the cost for that, I will bear the freaking cost." "Graphify was just an example of how front matter can do better than the spec file, knowledge graph, knowledge file, spec, and other Markdown files. These are the preprocessing files for your agent to check and then start the work." "Let's have the final plan, and then immediately I will review that and approve that, and we can start building."

**The shipped code, as last recorded.** A Next.js app with a Tauri v2 desktop build, hexagonal modules, `npm run verify` green at 1,596 passing tests and 6 expected failures on 13 September, a corpus gate of 8,513 byte-pinned files, a specs harness under `specs/`. Review state does not exist in the code. The splice engine has two measured defects named in the plan's section 17. Persistence keys keep a legacy `sgnk-md` prefix on purpose. Re-run the gates yourself; the numbers above are a starting point, not a result.

**Numbers the plan inherits from earlier rounds.** Anthropic API prices, Razorpay fees, the RBI mandate rules, the rupee rate used (₹95.96 to the dollar), Firestore's Mumbai region, R2 pricing, mammoth for Word, Drive's markdown export, GitHub's unauthenticated rate limit, Vercel's Python runtime, the prices of CodeGuide, ChatPRD and Kiro, Spec Kit's most-reacted issue, OpenAI's retirement of canvas on 28 May 2026, Octoverse's India figure, Zoho and Google Workspace India prices, Front Matter CMS's install count, and an unchecked Indian trademark register. They are in the 2026-09-16 reports and in `MVP0-PLAN-v3.md`'s Sources. Every one of them is a month or more old by the time anyone builds; check the ones the plan's decisions rest on.

**The studio's writing and interface rules, which the plan must obey and which you should apply to your own report.** British spelling. No em dashes or en dashes. Plain sentences; good vocabulary stays. Every number re-derived at write time, never carried forward. Every citation real and checkable. Icons are Google Material Symbols as inline SVG only; no emoji as interface; no other icon set. The gate: `python3 ~/Desktop/GitHub/sgnkai/scoring/gate.py --file <f> --strict`.

**What "done" means for this audit.** The founders will read your verdict, your blockers and your questions first, then the register. They will build from the plan plus your fixes. A finding they cannot act on is noise. A finding you did not make because it seemed obvious is the one that costs them a month.
...
Pass nine, readiness for the dev plan, build or buy, closure of earlier findings, distance from the code, vendor dependency, scenarios and the pre-mortem, founder capacity (A46 to A52).
Pass ten, the scorecard and the verdict.
...
The founders write the dev plan next. Does the product plan give it what it needs: acceptance criteria per screen, a data model, API contracts, the format specifications from A34, the permission matrix from A32, non-functional targets from A36, a definition of done tied to `specs/` and the verify gates, and a phase order with dependencies. List every input the dev plan would have to invent, and say which of those the founders must decide rather than a developer.

### A47. Build, buy or adopt

Every technical choice with lock-in: the editor engine for Doc mode (CodeMirror as shipped, or ProseMirror and Tiptap, or both), live collaboration (Yjs on Durable Objects, Liveblocks, or the studio's own journal), search (Postgres full text, Typesense, Meilisearch), auth (Supabase, Auth.js, Firebase, Clerk), payments (Razorpay, Dodo, Stripe India), email, analytics, PDF (Paged.js, Pandoc, Chrome), OCR, diagrams. For each: what the plan chose, what it costs to leave later, what the studio already runs elsewhere, and whether the choice is made or deferred.

### A48. Closure of earlier findings

The repository holds earlier audits and ledgers: `docs/GAPS-2026-09-08.md`, the 9 September briefing and verified list, the PRD's sections 32 and 57, the memory notes' list of shipped defects, and the decision cards. For each finding recorded there that still applies to the product as v4 describes it: closed by v4, still open, or silently dropped. Almanac's shutdown, the Zed Delta rebuttal owed, the attention literature against percentage badges, the fabricated quotations, the unwired engine, the sibling-vault default, the twenty hand-made kits.

### A49. Distance from the code as it is

Take each phase in section 18 and each "ships today" claim, and measure the distance from the shipped code: what exists, what is a stub, what does not exist, what must be removed. Run the gates read-only and report their real state today. Say how much of phase A is already there, and whether the plan's appetite of two weeks for it is honest against the code.

### A50. Vendor and account dependency

Sign-in is Google or GitHub only. If the Google OAuth app is suspended, every user is locked out; if the GitHub App is revoked, the funnel's write-back dies; if the Vercel or Supabase or Cloudflare account is paused for a billing or policy reason, the product is down and the files are unreachable. List every single point of failure by vendor, what the plan offers as a fallback (an email magic link, a second region, an export), and what it should. Include the model providers, the domain registrar and the payment processor.

### A51. Scenarios and a pre-mortem

Write three twelve-month scenarios with numbers: best, base and worst. Then the pre-mortem: it is September 2027 and frontmatter has been shut down. Write the five most likely reasons, each traced to a decision in the plan and to the evidence that should have warned the founders. Then the reverse: it worked; what had to be true.

### A52. Founder capacity and calendar

Two founders, client work alongside, about 1.2 days a week measured on this product. Who builds what in each phase, what needs a skill neither has shown in the repositories (Durable Objects appears nowhere in the studio's record; native desktop signing; live editing), what the calendar allows before the pilot, and what must be bought or contracted. Say plainly whether the plan can be built by the people who will build it, and what changes if it cannot.

## 6. The report

Write `verify/2026-09-17/CLAUDE-AUDIT-REPORT.md`, structured exactly like this. Alongside it, keep `verify/2026-09-17/CLAUDE-AUDIT-FINDINGS.jsonl`, one object per finding, appended as each is found, with these keys: `id` (F001 upward), `severity` (blocker, major, minor), `angle` (A1 to A52), `location` (file and line, screen id, or section), `claim`, `evidence`, `impact`, `fix`, `effort_days`, `verified` (true, false), `source` (URL or path), `checked_on` (date).

1. **Verdict.** Three sentences. Approve, approve with changes, or do not approve, and the one reason that decides it.
2. **Scorecard.** A table of the fifty-two angles: angle, score 0 to 5, top finding in one line, evidence location.
...
8. **Angle by angle.** A5 to A26 and A29 to A52, each with your reasoning and evidence, numbered as above.
...
16. A final fenced JSON block: `{"verdict": "...", "blockers": n, "majors": n, "minors": n, "claims_checked": n, "mismatches": n, "stale": n, "unverifiable": n, "asks_traced": 36, "asks_missing": n, "screens_reviewed": 30, "missing_screens": n, "angles_scored": 52, "mean_score": x.x}`.
