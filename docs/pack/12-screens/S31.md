...
Keep control | `C061` | One per pane, directly under that version | Two of them, so a choice is never made from a menu
Keep both | `C061` | One control in the footer bar | Makes two files, and names them
Let AI decide | `C061` | One control in the footer bar, beside Keep both, marked as the AI action | Available, never hidden. `[Z]` `docs/mvp0/SCREEN-CHANGES-2026-09-18.md:202`
...
Proposal review | `C119` | The change queue, opened on the proposal the AI made | Not a part of this screen. It is where Let AI decide sends you
...
Keep this one, left | The left pane's control | That version becomes the head. The other becomes a history entry | `E106` on a failed write. Nothing changes and the conflict stands | `conflict.resolved`
Keep this one, right | The right pane's control | The same, mirrored | `E106` | `conflict.resolved`
...
Let AI decide | The footer control | A merge is proposed. Every span it would change becomes a change queue item. **The file is not written.** The person is taken to the queue | `E080` when no provider answers, which is S32's condition. The four other resolutions stay available and nothing is spent | `conflict.ai.merge.proposed`
