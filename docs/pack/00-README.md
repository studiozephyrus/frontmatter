...
**Before development: the founder answers the checklist**, 75 items with the calendar first,
at `https://claude.ai/artifact/N9hkuuZsMRnjxqvxyvdEPH`. Its source is `review/checklist.html`, built by
`tools/build-checklist.py` from `review/checklist-items-44.json` and `review/checklist-items-19sep.json`.
Answers are read back from the page's store, collection `answers`. Batch 2's spec lanes are already
written in `specs/`.
