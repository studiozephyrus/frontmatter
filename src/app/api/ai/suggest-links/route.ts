...
export async function POST(req: Request): Promise<Response> {
  ...
  try {
    body = (await req.json()) as { text?: string; candidates?: string[] };
  } catch {
  ...
  if (!Array.isArray(body.candidates) || body.candidates.length === 0) {
    return new Response(
      JSON.stringify({ error: "bad_request", detail: "candidates required" }),
      { status: 400, headers: JSON_HEADERS },
    );
  }
  ...
  const candidates = body.candidates.filter((c): c is string => typeof c === "string");

  try {
    const result = await container.suggestLinks({ text: body.text, candidates });
