...
constants in the source** (`docs/mvp0/PRODUCT-PLAN.md:1816`). That turns most of the open questions
...
**So `:307` supersedes `:213`.** Anyone reading the change register top to bottom will meet the
deferral first, and it is no longer true.
...
(`docs/mvp0/PRODUCT-PLAN.md:1817`). They stop being decisions the build waits on and become rows
with defaults.

| Q | The question | Where it lands | Rows |
|---|---|---|---|
| 2 | The Pro price and the routing | S35 and S36 | `price.pro.*`, `routing.*` |
...
`docs/mvp0/PRODUCT-PLAN.md:1763`, and those eight bullets cover ten questions, not eleven. The
...
(`docs/mvp0/PRODUCT-PLAN.md:1771`).
...
Every row below carries the same eight columns. Where a column would be identical for a whole block
it is stated once above the table rather than repeated.

Column | What it holds
**Key** | The dotted identifier the code reads. Lower case, dotted, never renamed. A rename is a new key plus a migration
...
**Default** | The value the row ships with, from the plan's recommendation
**Free** and **Pro** | The per-plan value where the row is per plan
...
### 4.1 Documents, pages and people

| Key | Type | Bounds | Free | Pro | On lowering |
|---|---|---|---|---|---|
| `limits.docs.cloud` | `int \| null` | 1 to 100,000, or null | **50** | **null** | S33 over the cap. Every document stays readable and exportable. Nothing new is created until under the cap |
| `limits.pages.published` | `int \| null` | 0 to 10,000, or null | **5** | **null** | Existing pages keep serving. No new page is published until under the cap. **A published page is never taken dark by a limit change**, because a stranger's link would break |
| `limits.collab.live` | `int \| null` | 0 to 100, or null | **1** | **null** | A session already open finishes. No new session starts over the cap |
| `limits.history.days` | `int` | 0 to 3,650 | **7** | **90** | Versions outside the new window are pruned to the head **after a 30-day grace**, never on save. See 4.5 |
| `limits.trash.days` | `int` | 1 to 365 | **30** | **30** | Same 30-day grace |

**The `limits.collab.live` default is 1 and it moved on 18 September.** The market says three, and
the founders chose one on cost: a live session is a Durable Object held open for as long as two
people are in it, and that is the one free-tier cost that scales with time rather than with calls
(`docs/mvp0/PRODUCT-PLAN.md:1106`).

**A contradiction the panel inherits.** Section 18's data model still carries `3 live on Free` at
`docs/mvp0/PRODUCT-PLAN.md:1438`, against section 13's decided 1. **The panel row is the answer**,
...
| Key | Type | Bounds | Free | Pro | On lowering |
|---|---|---|---|---|---|
| `limits.upload.file.mb` | `int` | 1 to 200 | **5** | **25** | Existing files stay. A new upload over the new size is refused with the size named |
| `limits.upload.total.gb` | `int \| null` | 1 to 1,000, or null | **1** | **10** | S33. Nothing is deleted |

**A hard ceiling the panel may not cross.** `limits.upload.file.mb` cannot be set above 100, because
`docs/mvp0/PRODUCT-PLAN.md:1340` records that Workers cap a request body at 100 MB and the plan
routes large uploads straight to R2 with a presigned URL. The bound of 200 above assumes that
presigned path exists. **Until it does, the bound is 100.** Say which is live in the row's help text.

### 4.3 AI

| Key | Type | Bounds | Free | Pro | On lowering |
|---|---|---|---|---|---|
| `limits.ai.edits.month` | `int \| null` | 0 to 100,000, or null | **10** | **100** | The bucket's capacity falls. **Credits already spent are never clawed back** |
| `limits.ai.blueprints.month` | `int \| null` | 0 to 10,000, or null | **1** | **5** | Same |
| `limits.ai.rewrites.blueprint` | `int \| null` | 0 to 50, or null | **3** | **null** | Applies to the next blueprint, never to one in flight |
| `limits.ideas.depth.max` | `enum` | `low`, `medium`, `high` | **low** | **high** | An existing blueprint at a higher depth stays readable |
| `limits.ai.bucket.refill` | `enum` | `monthly`, `continuous` | **continuous** | **continuous** | See 4.4 |
| `limits.ai.newaccount.days` | `int` | 0 to 365 | **7** | **0** | The starting-allowance window of file 27 section 9.3 |
| `limits.ai.newaccount.edits` | `int` | 0 to 100 | **3** | n/a | Same |
| `limits.ai.newaccount.blueprints` | `int` | 0 to 100 | **0** | n/a | Same |

**Top-ups are counts, not limits**, and they add to the bucket rather than raising it.

| Key | Type | Bounds | Default |
|---|---|---|---|
| `topup.edits.count` | `int` | 1 to 10,000 | **50** |
| `topup.blueprints.count` | `int` | 1 to 1,000 | **3** |

### 4.4 The refill row is not cosmetic

`limits.ai.bucket.refill` defaults to `continuous`, which means a token bucket rather than a calendar
counter, and the arithmetic in file 27 section 9.1 is why.
...
**Setting this row to `monthly` is allowed and is a mistake.** The row carries that sentence in its
help text, with a link to file 27 section 9.1.

### 4.5 History is the one row where lowering deletes something

Every other `limits.*` row leaves data alone and refuses new work. `limits.history.days` is
...
| Key | Type | Bounds | Free | Pro | On lowering |
|---|---|---|---|---|---|
| `limits.github.repos` | `int \| null` | 0 to 1,000, or null | **1** | **null** | Existing connections keep working. No new repository is connected |
| `limits.github.pushes.month` | `int \| null` | 0 to 100,000, or null | **20** | **null** | Bucket capacity falls. Pushes already made stand |
| `limits.drive.enabled` | `bool` | | **true** | **true** | It is the person's own storage and costs us nothing |
| `limits.links.password` | `bool` | | **false** | **true** | An existing password link keeps working. No new one is created |
| `limits.branding.removable` | `bool` | | **false** | **true** | The Made with line returns to pages on a downgrade |
| `limits.portfolio.enabled` | `bool` | | **false** | **true** | The portfolio goes private rather than being deleted |

**The GitHub rows cannot be moved behind a higher tier**, whatever the numbers say. The plan's reason
is at `docs/mvp0/PRODUCT-PLAN.md:1110`: the connection is the product.

### 4.7 Prices

| Key | Type | Bounds | Default | Read by |
|---|---|---|---|---|
| `price.pro.monthly.inr` | `money.inr` | 0 to 1,500,000 paise | **₹299**, GST inclusive | S29 plan page, and the Razorpay call |
| `price.pro.annual.inr` | `money.inr` | 0 to 1,500,000 paise | **₹2,499**, GST inclusive | Same |
| `price.topup.edits.inr` | `money.inr` | 0 to 1,500,000 paise | **₹99** | Same |
| `price.topup.blueprints.inr` | `money.inr` | 0 to 1,500,000 paise | **₹149** | Same |

**The upper bound is not arbitrary.** `₹15,000` per transaction is an architectural constant set by
the Reserve Bank, and it is the mandate ceiling, so a price above it cannot be charged on a recurring
mandate at all. **The panel refuses the save rather than letting a price be set that Razorpay will
decline.**

**Money is stored in paise as an integer.** A price row that holds a float is a defect.
...
shape, so a founder reordering a chain sees the money move. Those figures are re-derived from file 27
section 8 and are not carried forward.
...
| Key | Type | Bounds | Default | Read by |
|---|---|---|---|---|
| `exception.<accountId>.<limitKey>` | the limit's own type | the limit's own bounds | none | `limitsFor(account)`, which resolves plan plus exception |
| `exception.<accountId>.<limitKey>.expiresAt` | `date` | **required, and at most 365 days out** | none | the same |
| `exception.<accountId>.<limitKey>.reason` | `string` | 1 to 500 characters, **required** | none | the audit row |

**An exception with no expiry is refused.** The plan requires it to expire
(`docs/mvp0/PRODUCT-PLAN.md:1859`), and a permanent exception is a plan change wearing a disguise.
...
|---|---|---|---|
| `pilot.stop.activeWeek2` | `int` | **4** of 20 | `docs/mvp0/PRODUCT-PLAN.md:1743` |
| `pilot.stop.kickoffRuns` | `int` | **2** of 10 | the same |
| `pilot.stop.namesProblem` | `int` | **3** of 20 | the same |
| `pilot.continue.activeWeek2` | `int` | **6** of 20 | `docs/mvp0/PRODUCT-PLAN.md:1749` |
...
(`docs/mvp0/PRODUCT-PLAN.md:1844`). Two of them are shown on S37 **as locked rows with the reason on
...
- It takes the account, reads its plan row, applies any unexpired exception, and returns the limit
  set (`docs/mvp0/PRODUCT-PLAN.md:1833`).
...
(`docs/mvp0/PRODUCT-PLAN.md:1840`): everything stays readable and exportable, and nothing new is
...
1. **Compute the blast radius before saving.** How many accounts the change moves over their cap, and
   **name them**. The plan requires the count and the names
   (`docs/mvp0/PRODUCT-PLAN.md:1842`).
...
| Over `limits.upload.total.gb` | Every upload downloads | No new upload |
| Over `limits.ai.edits.month` | Everything that never touches a model, which is most of the editor | The AI box says the entitlement message, **never the outage message** |
...
**Every change writes one row** (`docs/mvp0/PRODUCT-PLAN.md:1854`): who, which setting, from what, to
...
- **Super admin is a flag on an account, not a plan**, and both founders hold it
  (`docs/mvp0/PRODUCT-PLAN.md:1852`).
...
- The `limits.upload.file.mb` upper bound, which depends on whether the presigned R2 upload path
...
- That `continuous` is the right default for `limits.ai.bucket.refill`. The arithmetic in file 27
...
- That eleven questions genuinely stop being decisions. A row with a default is a decision somebody
  ...
  (`docs/mvp0/PRODUCT-PLAN.md:1773`); the panel only moves when.
